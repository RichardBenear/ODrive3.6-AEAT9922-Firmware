...
void Encoder::setup() {
    ...
    // MA732 - Clock polarity = high, sample MISO on rising edge
    // AEAT9966, AEAT9922, AMS232 - Clock polarity = low, sample MISO on falling edge
    spi_task_.config = {
        .Mode = SPI_MODE_MASTER,
        .Direction = SPI_DIRECTION_2LINES,
        .DataSize = SPI_DATASIZE_8BIT, //AEAT18 support
        .CLKPolarity = (mode_ == MODE_SPI_ABS_AEAT || mode_ == MODE_SPI_ABS_MA732)
                ? SPI_POLARITY_HIGH
                : SPI_POLARITY_LOW,
    ...
    if (mode_ == MODE_SPI_ABS_AEAT9922) {
        abs_spi_dma_length_ = 3;
    } else {
        abs_spi_dma_length_ = 2;
    }
...
bool Encoder::abs_spi_start_transaction() {
    if (mode_ & MODE_FLAG_ABS){
        if (Stm32SpiArbiter::acquire_task(&spi_task_)) {
            ...
            spi_task_.length = abs_spi_dma_length_;
...
// Returns 0 if even parity over nbits, 1 if odd parity.
// (Compute XOR-fold parity over the lower nbits of v.)
static inline uint8_t parity_n(uint32_t v, uint8_t nbits) {
    if (nbits < 32) {
        v &= ((1u << nbits) - 1u);
    }
    v ^= v >> 16;
...
void Encoder::abs_spi_cb(bool success) {
    ...
    switch (mode_) {
        case MODE_SPI_ABS_AMS: {
            ...
            uint16_t rawVal = (abs_spi_dma_rx_[0] << 8) | abs_spi_dma_rx_[1];
        ...
        // AEAT9922-18 SPI4 mode Support ======================================
        case MODE_SPI_ABS_AEAT9922: {
            // 24 bits received MSB-first (3 bytes)
            // bits 17..0 : POS[17..0]
            // bit 18     : Error Flag
            // bit 19     : Even Parity
            // bits 23..20: don't care / padding
            uint32_t raw24 =
                ((uint32_t)abs_spi_dma_rx_[0] << 16) |
                ((uint32_t)abs_spi_dma_rx_[1] << 8)  |
                ((uint32_t)abs_spi_dma_rx_[2] << 0);

            // bad parity or error, exit
            if (parity_n(raw24, 20) != 0) goto done;
            if ((raw24 >> 18) & 1u) goto done;

            pos = raw24 & 0x3FFFFu; // extract the position
        } break;
